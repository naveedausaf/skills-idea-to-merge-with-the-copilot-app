...
- Bookmarks are persisted in the browser using `localStorage`.
- Browser-only code that touches `localStorage` must run behind a `client:load` hydration boundary, so server-side rendering and the static build never touch browser APIs.
